...
void readFrom(const uint8_t address) {

	// slave select - begin data transfer

	pinWriteLow(B, 4);



	// tell address and read mode (MSb 0)

	uint8_t readAddr = address & 0x7F;

	SPI_Transmission(&readAddr, 1);

}


void writeTo(const uint8_t address) {
	...
	// tell address and write mode (MSb 1)

	uint8_t writeAddr = address | 0x80;

	SPI_Transmission(&writeAddr, 1);

}


void dataExchange(uint8_t *dataBuffer, const uint8_t length) {

	SPI_Transmission(dataBuffer, length);



	// slave select - end data transfer
...
void readBurst(uint8_t address, uint8_t *readBuffer, uint8_t length) {
	...
	dataExchange(readBuffer, length);
